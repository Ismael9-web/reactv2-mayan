  ...
  try {
    ...
    for (const docId of documentIdsFromType3) {
      try {
        ...
        let documentDateFromMetadata = null; // Variable to store the "Date" metadata value
        const allMetadataForDoc = {}; // To store all metadata for the current document

        // Iterate through metadata results to find "Document_ID" and "Date", and collect all other metadata
        for (const meta of metadataResponse.results) {
          ...
          if (meta.metadata_type.label === 'Date') {
            documentDateFromMetadata = meta.value; // Store the Date value
          }
        }
        ...
        // AND if the Date value's month and year match the current month and year
        let isCurrentMonthAndYear = false;
        if (documentDateFromMetadata) {
          const docDate = new Date(documentDateFromMetadata);
          if (docDate.getMonth() === currentMonth && docDate.getFullYear() === currentYear) {
            isCurrentMonthAndYear = true;
          }
        }

        if (documentIdFromMetadata && documentIdsFromType2.has(documentIdFromMetadata) && isCurrentMonthAndYear) {
          paymentStatus = 'paid';
